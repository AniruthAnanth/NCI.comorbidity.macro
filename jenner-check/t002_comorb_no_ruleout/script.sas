...
%mend; 
*** End macro COMORB;
/* Caller: exercises %COMORB with RULEOUT=N, the macro's non-ruleout code
   path (every in-window claim counts, MedPAR or Outpatient alike -- no
   30-day confirmation requirement). Mock claims data stands in for a real
   Medicare claims extract. */
data claims;
 ...
 startdate = mdy(1,1,2020); enddate = mdy(12,31,2020);

 /* Patient 10: single Outpatient claim for chronic pulmonary disease --
    would need confirmation under RULEOUT=Y, but counts immediately here. */
 patient_id=10; claimstartdate=mdy(2,1,2020); claimenddate=mdy(2,1,2020);
 claimtype='O'; icd_dgns_cd1='J45'; output;

 /* Patient 10: also a peptic ulcer diagnosis on a Carrier (NCH) claim. */
 patient_id=10; claimstartdate=mdy(8,15,2020); claimenddate=mdy(8,15,2020);
 claimtype='N'; icd_dgns_cd1='K26'; output;

 /* Patient 11: dementia on a single Outpatient claim. */
 patient_id=11; claimstartdate=mdy(5,1,2020); claimenddate=mdy(5,1,2020);
 claimtype='O'; icd_dgns_cd1='G30'; output;
run;

%COMORB(claims,patient_id,startdate,enddate,claimstartdate,claimenddate,claimtype,icd_dgns_cd1,N,comorbidities);

proc print data=comorbidities label noobs;
 var patient_id Charlson NCI_index copd ulcers dementia;
run;
