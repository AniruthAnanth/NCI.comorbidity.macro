...
%mend; 
*** End macro COMORB;
/* Caller: exercises %COMORB with mock Medicare-style claims data, since the
   macro itself expects a pre-built claims dataset rather than shipping one.
   Mixes ICD-9 (pre-10/1/2015) and ICD-10 (post) coded claims across three
   patients, MedPAR and Outpatient claim types, and the RULEOUT algorithm. */
data claims;
 length patient_id 8 startdate enddate claimstartdate claimenddate 8
        claimtype $1 icd_dgns_cd1 $7;
 format startdate enddate claimstartdate claimenddate mmddyy10.;

 /* Patient 1: ICD-9 era, window 2014. Acute MI + diabetes w/ complications,
    both on MedPAR claims -- confirmed, no ruleout needed. */
 startdate = mdy(1,1,2014); enddate = mdy(12,31,2014);
 patient_id=1; claimstartdate=mdy(3,10,2014); claimenddate=mdy(3,12,2014);
 claimtype='M'; icd_dgns_cd1='4109'; output;
 patient_id=1; claimstartdate=mdy(6,1,2014); claimenddate=mdy(6,2,2014);
 claimtype='M'; icd_dgns_cd1='2504'; output;

 /* Patient 2: ICD-10 era, window 2019. COPD on MedPAR, plus rheumatologic
    disease on Outpatient confirmed by two claims >30 days apart. */
 startdate = mdy(1,1,2019); enddate = mdy(12,31,2019);
 patient_id=2; claimstartdate=mdy(2,15,2019); claimenddate=mdy(2,16,2019);
 claimtype='M'; icd_dgns_cd1='J44'; output;
 patient_id=2; claimstartdate=mdy(4,1,2019); claimenddate=mdy(4,1,2019);
 claimtype='O'; icd_dgns_cd1='M06'; output;
 patient_id=2; claimstartdate=mdy(9,15,2019); claimenddate=mdy(9,15,2019);
 claimtype='O'; icd_dgns_cd1='M06'; output;

 /* Patient 3: ICD-10 era, window 2019. Single unconfirmed outpatient
    AIDS-coded claim (should be ruled out) plus a confirmed MedPAR
    cerebrovascular disease claim. */
 startdate = mdy(1,1,2019); enddate = mdy(12,31,2019);
 patient_id=3; claimstartdate=mdy(5,5,2019); claimenddate=mdy(5,5,2019);
 claimtype='O'; icd_dgns_cd1='B20'; output;
 patient_id=3; claimstartdate=mdy(7,20,2019); claimenddate=mdy(7,22,2019);
 claimtype='M'; icd_dgns_cd1='I63'; output;
run;

%COMORB(claims,patient_id,startdate,enddate,claimstartdate,claimenddate,claimtype,icd_dgns_cd1,Y,comorbidities);

proc print data=comorbidities label noobs;
 var patient_id Charlson NCI_index acute_mi diabetes_comp copd rheum_disease cvd aids;
run;
